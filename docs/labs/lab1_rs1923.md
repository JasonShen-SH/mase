...
The pass **profile_statistics_analysis_pass** focuses on collecting numerical information (e.g., parameters & activations) of the graph's specific modules. These specific modules are usually given by a configuration file.

In our case, we focus on collecting statistics of weights (mean and variance) of Linear layer and activation (max, min, and range) of ReLU.

The datails of the computing and updating process can be found at **stat.py** under the same directory.
