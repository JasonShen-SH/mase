...
We also randomly generated an input for the linear layer and passed it through both the original linear layer and the weight-quantized linear layer. It can be observed that their outputs are different.
...
def calculate_flop_for_relu(module, batch_size):
    # per element comparison with 0 (in essence, a minus)
    input_features = module.input_features*batch_size
